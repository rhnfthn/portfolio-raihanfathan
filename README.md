...
## Local Development (Postgres Local via Supabase)

If you don't want to use Supabase Cloud yet but still want a local PostgreSQL database, use **Supabase Local** (runs via Docker).
...
- Supabase CLI will apply migrations in `supabase/migrations/`.
- The initial schema is also available in `supabase/schema.sql`.
...
This same script is also available in `supabase/schema.sql` in this repository.
...
This script is also available in `supabase/admin.sql`.
...
⭐ If this project helped you, consider giving it a star on GitHub!# portfolio-raihanfathan
